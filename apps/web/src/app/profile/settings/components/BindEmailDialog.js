...
export function BindEmailDialog({
  ...
  const requirePassword = settings.email_change_requires_password?.value && user?.hasPassword;
  ...
  const canProceedToNextStep = () => {
    switch (step) {
      case 1:
        return formData.email.trim() && formData.codeSent && formData.code.trim();
      case 2:
                ...
                <>
                  <div className='p-4 bg-muted rounded-lg'>
                    <p className='text-sm text-muted-foreground'>
                      验证码已发送到：<span className='font-medium text-card-foreground'>{formData.email}</span>
                    </p>
                  </div>
                  <div>
                    <Label className='text-sm font-medium text-card-foreground block mb-2'>

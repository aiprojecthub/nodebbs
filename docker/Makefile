...
help: ## 显示帮助信息
	...
	@echo ""
	@echo "$(YELLOW)环境配置:$(NC)"
	@echo "  使用 ENV=prod 切换到生产环境，例如: $(GREEN)ENV=prod make up$(NC)"
	@echo "  默认环境: $(GREEN)开发环境 (dev)$(NC)"
	@echo ""
	@awk 'BEGIN {FS = ":.*##"; printf "使用方法: make $(GREEN)<target>$(NC) [ENV=dev|prod]\n\n"} /^[a-zA-Z_-]+:.*?##/ { printf "  $(GREEN)%-15s$(NC) %s\n", $$1, $$2 } /^##@/ { printf "\n$(BLUE)%s$(NC)\n", substr($$0, 5) } ' $(MAKEFILE_LIST)
...
up: ## 启动所有服务 (可用 ENV=prod 指定生产环境)
	@echo "$(BLUE)启动所有服务 [$(ENV_NAME)]...$(NC)"
	$(COMPOSE_CMD) up -d
	@echo "$(GREEN)服务已启动!$(NC)"
...
down: ## 停止所有服务
	@echo "$(BLUE)停止所有服务 [$(ENV_NAME)]...$(NC)"
	$(COMPOSE_CMD) down
	@echo "$(GREEN)服务已停止$(NC)"

restart: ## 重启所有服务
	@echo "$(BLUE)重启所有服务 [$(ENV_NAME)]...$(NC)"
	$(COMPOSE_CMD) restart
	@echo "$(GREEN)服务已重启$(NC)"

build: ## 重新构建镜像
	@echo "$(BLUE)重新构建镜像 [$(ENV_NAME)]...$(NC)"
	$(COMPOSE_CMD) build --no-cache
	@echo "$(GREEN)镜像构建完成$(NC)"

rebuild: ## 重新构建并启动
	@echo "$(BLUE)重新构建并启动服务 [$(ENV_NAME)]...$(NC)"
...
seed-reset: ## 重置并重新初始化数据
	...
	echo; \
	if [[ $$REPLY =~ ^[Yy]$$ ]]; then \
		$(COMPOSE_CMD) exec api npm run seed:reset; \
...
clean: ## 停止并删除所有容器、网络
	@echo "$(YELLOW)停止并删除所有容器和网络 [$(ENV_NAME)]...$(NC)"
	$(COMPOSE_CMD) down
	@echo "$(GREEN)清理完成$(NC)"
...
exec-db: ## 进入数据库容器
	$(COMPOSE_CMD) exec postgres psql -U postgres -d nodebbs

exec-redis: ## 进入 Redis 容器
	$(COMPOSE_CMD) exec redis redis-cli

health: ## 检查服务健康状态
	@echo "$(BLUE)检查服务健康状态 [$(ENV_NAME)]...$(NC)"
